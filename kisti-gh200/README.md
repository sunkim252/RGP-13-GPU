...
AmgX 분산도 fgm 스택 전용(`gpuPEqnSolver amgx;`) — rd0110 이송 후
4랭크로 수렴 정상 여부만 확인 (성능 기본값은 pcg).
...
## x86(RTX 4060, discrete) 사전 검증 매트릭스 — 2026-07-13 야간

| 검증 항목 | 결과 |
|---|---|
| 자동 감지 (pmaHPT=0 → copy) | ✅ 배너 "copy (discrete) [coherent HW: no]" |
| mode1(mapped) vs mode0 — 화학-only 직렬 | ✅ 전 필드 비트-동일 |
| mode1 vs mode0 — 화학-only 4랭크 | ✅ 전 랭크 비트-동일 |
| mode1 vs mode0 — 풀-GPU 직렬 | ✅ 결정론 필드 집합에서 비트-동일 |
| RGP_GPU_UNIFIED=2 강제(비-코히런트) | ✅ mapped 강등 경고 + 비트-동일 |
| DLB on/off 4랭크 | ✅ 비트-동일 |
| fgm 스택(SRK 106종, rd0110) 생존+성능 | ✅ 34.2 s/step (기준 동등) |
| fgm 풀스택(rd0110) mode1 생존+성능 | ✅ 완주, 194s vs 193s(mode0) — 비트 판정은 LTS 전파로 불가(케이스 전체가 노이즈 클래스; fgm 비트 정합은 소형 게이트가 담당) |
| 스모크 1·2A·2B·3 스크립트 로직 통주행 | ✅ 5/5 PASS |

**GH200 실기에서 새로 확인되는 것은 단 하나 — CUDA 런타임의 진짜
코히런트(native) 시맨틱**. 나머지 전 경로는 x86에서 닫혀 있다.

추가(2026-07-13): **aarch64 실컴파일 검증 완료** — GitHub Actions
ubuntu-24.04-arm 러너에서 CUDA 12.6 sbsa 툴킷 + GH200 def 플래그
(sm_90+PTX, -fmad=false)로 Foam-free CUDA TU 4종 전부 컴파일 성공
(.github/workflows/arm64-cuda-smoke.yml — push마다 자동 재검증).
디바이스 스택의 arm64 이식성은 하드웨어 없이 이미 실증됨.
